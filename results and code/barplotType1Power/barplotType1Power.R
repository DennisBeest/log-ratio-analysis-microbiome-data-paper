...
#---to prevent a warning. It is actually NA.
data[33, 6] <- 0
...
for(i in 1:6)
{
	...
	if(i == 1) index <- data$sd == 0.25 & data$Filt == ""
	if(i == 2) index <- data$sd == 1 & data$Filt == ""

	if(i == 3) index <- data$sd == 0.25 & data$Filt == ""
	if(i == 4) index <- data$sd == 1 & data$Filt == ""

	if(i == 5) index <- data$sd == 0.25 & data$Filt == ""
	if(i == 6) index <- data$sd == 1 & data$Filt == ""
...
grob1 <- grobTree(textGrob(text1, x=xloc1,  y=0.96, gp=gpar(fontsize = fontSize), hjust = 0))

text2 <- expression(paste("(B) ","Type 1 ; ", sigma[a], " = 1"," ; ", gamma, " = 0", ""))
grob2 <- grobTree(textGrob(text2, x=xloc1,  y=0.96, gp=gpar(fontsize = fontSize), hjust = 0))

text3 <- expression(paste("(C) ","Power ; ", sigma[a], " = 0.25"," ; ", gamma, " = 0", " ; FC = 1.5"))
grob3 <- grobTree(textGrob(text3, x=xloc1,  y=0.96, gp=gpar(fontsize = fontSize), hjust = 0))

text4 <- expression(paste("(D) ","Power ; ", sigma[a], " = 1"," ; ", gamma, " = 0", " ; FC = 1.5"))
grob4 <- grobTree(textGrob(text4, x=xloc1,  y=0.96, gp=gpar(fontsize = fontSize), hjust = 0))

text5 <- expression(paste("(E) ","Type 1 ; ", sigma[a], " = 0.25"," ; ", gamma, " = 2"))
grob5 <- grobTree(textGrob(text5, x=xloc1,  y=0.96, gp=gpar(fontsize = fontSize), hjust = 0))

text6 <- expression(paste("(F) ","Type 1 ; ", sigma[a], " = 1"," ; ", gamma, " = 2"))
grob6 <- grobTree(textGrob(text6, x=xloc1,  y=0.96, gp=gpar(fontsize = fontSize), hjust = 0))
...
nameFile <- "barplotType1Power/barplotType1Power.pdf"
savefile <- TRUE
if(savefile)
{
